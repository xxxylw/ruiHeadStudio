...
RUN_ROOT="$ROOT/outputs/text_gs_render_reference_sweep_20260830"
...
launch_variant() {
  local gpu="$1" tag="$2" lambda_reference="$3"
  ...
  CUDA_VISIBLE_DEVICES="$gpu" nohup python3 launch.py --config configs/headstudio_retry.yaml --train \
    ...
    "system.lambda_frequency_quality=0.0" "system.lambda_rendered_reference=$lambda_reference" \
...
launch_variant 0 reference_q001 0.001
launch_variant 1 reference_q003 0.003
launch_variant 2 reference_q006 0.006
